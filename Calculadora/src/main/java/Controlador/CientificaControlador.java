...
public class CientificaControlador implements Initializable {
    @javafx.fxml.FXML
    ...
    private String auxiliarTrigonometria = "";
    ...
    public void agregarOperacion(ActionEvent actionEvent) {
        ...
        if (!lbDisplaySecundario.getText().contains("(")) {
    ...
    public void calcular(ActionEvent actionEvent) {
        if (lbDisplaySecundario.getText().contains("=") || lbDisplaySecundario.getText().isEmpty() || lbDisplaySecundario.getText().contains("(")) {
            return;
        }
    ...
    private void bloquearBotones(boolean bloquear) {
        ...
        btnCos.setDisable(bloquear);
        btnSen.setDisable(bloquear);
        btnTan.setDisable(bloquear);
        btnExp.setDisable(bloquear);
    }

    @javafx.fxml.FXML
    public void operarTrigonometria(ActionEvent actionEvent) {
        String operacion = "";
        if (auxiliarTrigonometria.isEmpty()) {
            operacion = ((Button) actionEvent.getSource()).getText();
        } else {
            if (auxiliarTrigonometria.equals("s")) {
                operacion = "SIN";
            } else if (auxiliarTrigonometria.equals("c")) {
                operacion = "COS";
            } else if (auxiliarTrigonometria.equals("t")) {
                operacion = "TAN";
            }
            auxiliarTrigonometria = "";
        }
        if (!lbDisplaySecundario.getText().isEmpty() && !lbDisplaySecundario.getText().contains("(")) {
            calcular(actionEvent);
        }
        String resultado = Operaciones.operarTrigonometria(lbDisplayPrincipal.getText(), operacion);
        lbDisplaySecundario.setText(operacion.toLowerCase() + "(" + lbDisplayPrincipal.getText() + ")");
        lbDisplayPrincipal.setText(resultado);
    }
    ...
    public void keyPressed(KeyEvent event) {
        ...
        if (codigoPresionado == KeyCode.ESCAPE) {
        ...
        if (botonPresionado.toLowerCase().matches("[s|c|t]")) {
            auxiliarTrigonometria = botonPresionado.toLowerCase();
            operarTrigonometria(new ActionEvent());
            return;
        }
    ...
    @javafx.fxml.FXML
    public void irABasica(ActionEvent actionEvent) throws IOException {
        VBox panel = FXMLLoader.load(getClass().getResource("/Vista/calculadora_main.fxml"));
        vboxPrincipal.getChildren().setAll(panel);
        vboxPrincipal.getScene().getWindow().setWidth(360 + 16);
        vboxPrincipal.getScene().getWindow().setHeight(760 + 40);
    }

    @javafx.fxml.FXML
    public void irAConversor(ActionEvent actionEvent) throws IOException {
        VBox panel = FXMLLoader.load(getClass().getResource("/Vista/calculadora_conversion.fxml"));
        vboxPrincipal.getChildren().setAll(panel);
        vboxPrincipal.getScene().getWindow().setWidth(420 + 12);
        vboxPrincipal.getScene().getWindow().setHeight(620 + 40);
    }
